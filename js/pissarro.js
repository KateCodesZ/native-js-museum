async function fetchRenoirPaintings() {
  ...
  try {
    // // Hämta data om målningar av Camille Pissarro med medium "Paintings"
    const searchResponse = await fetch(
      "https://collectionapi.metmuseum.org/public/collection/v1/search?q=Camille Pissarro&medium=Paintings"
    );
    ...
    // Välj de första 20 målningarnas ID och filtrera bort målningen med ID 437303
    const paintingIDs = searchData.objectIDs
      .slice(0, 20)
      .filter((id) => id !== 437303);
...
async function fetchPaintingDetails(id) {
  ...
  // Kontrollera om målningen har en bild, att den är "oil on canvas och att konsträren är Camille Pissarro
  if (
    !data.primaryImageSmall ||
    !data.medium.toLowerCase().includes("oil on canvas") ||
    data.artistDisplayName !== "Camille Pissarro"
  ) {
    return null;
  }
