async function fetchRenoirPaintings() {
  ...
  try {
    // // Hämta data om målningar av Vincent van Gogh med medium "Paintings"
    const searchResponse = await fetch(
      "https://collectionapi.metmuseum.org/public/collection/v1/search?q=Vincent van Gogh&medium=Paintings"
    );
    ...
    // Välj ID för målningar mellan index ... och uteslut index 6, 10, 13
    const paintingIDs = searchData.objectIDs
      .slice(0, 17)
      .filter((_, index) => ![6, 10, 13].includes(index));
